...
def _(RandomWalkSimulator):
    ...
    assert (
        convert_D_to_P(convert_P_to_D(1.0)) == 1.0
    )  # check inverse transform always defined properly
...
def _(Lx, Ly, P_true, T, U_true, np, simulator):
    ...
    observed_column_counts, initial_positions, final_positions = simulator.simulate(
        U=U_true, P=P_true, T=T, random_seed=1234
    )
...
def _(
    ...
    test_counts = surrogate_pde_model(
        U=U_true, D=D_true, t=T, Lx=Lx, h=initial_region_half_width
    )
    ...
    axes.scatter(
        x_positions, observed_column_counts, alpha=0.7, color="skyblue", edgecolor="navy"
    )
...
def _(
    ...
    def abc_stochastic(observed_data, U_limits, P_limits, n_samples=100000, n_keep=1000):
        ...
        for i in tqdm.tqdm(range(n_samples)):
            ...
            # Run simulation
            sim_counts, _, _ = simulator.simulate(
                U=U_sample, P=P_sample, T=T, random_seed=None
            )
    ...
    if os.path.exists(abc_stochastic_file) and not force_rerun_abc_stochastic:
        ...
        print(
            f"✅ Loaded {abc_stochastic_results['n_keep']} samples from {abc_stochastic_file}"
        )
    else:
        # Run ABC with stochastic simulator
        abc_stochastic_results = abc_stochastic(
            observed_column_counts, U_limits, P_limits, n_samples=10000, n_keep=1000
        )
...
def _(
    ...
    if os.path.exists(abc_surrogate_file) and not force_rerun_abc_surrogate:
        ...
        print(
            f"✅ Loaded {abc_surrogate_results['n_keep']} samples from {abc_surrogate_file}"
        )
    else:
        # Run ABC with surrogate model
        abc_surrogate_results = abc_surrogate(
            observed_column_counts, U_limits, P_limits, n_samples=100000, n_keep=1000
        )
...
def _(
    ...
    mcmc_results_burned = {
        key: value[burn_in:] for key, value in mcmc_results.items() if key != "n_samples"
    }
    ...
    axes_hist[0, 0].hist(
        abc_stochastic_results["U"],
        bins=50,
        alpha=0.7,
        label="ABC Stochastic",
        density=True,
    )
    axes_hist[0, 0].axvline(
        U_true, color="red", linestyle="--", linewidth=2, label=f"True U={U_true}"
    )
    ...
    axes_hist[1, 0].axvline(
        D_true, color="red", linestyle="--", linewidth=2, label=f"True D={D_true}"
    )
    ...
    axes_hist[1, 1].hist(
        abc_surrogate_results["D"], bins=50, alpha=0.7, density=True, color="orange"
    )
    axes_hist[1, 1].axvline(
        D_true, color="red", linestyle="--", linewidth=2, label=f"True D={D_true}"
    )
    ...
    axes_hist[1, 2].hist(
        mcmc_results_burned["D"], bins=50, alpha=0.7, density=True, color="green"
    )
    axes_hist[1, 2].axvline(
        D_true, color="red", linestyle="--", linewidth=2, label=f"True D={D_true}"
    )
...
def _(
    ...
    axes_corr[0].scatter(
        ...
        s=100,
        marker="*",
        label=f"True: U={U_true}, D={D_true}",
        zorder=5,
    )
...
def _(
    ...
    posterior_samples_abc_stochastic = np.array(
        [abc_stochastic_results["U"], abc_stochastic_results["D"]]
    ).T
    create_professional_corner_plot(
...
def _(
    ...
    posterior_samples_abc_surrogate = np.array(
        [abc_surrogate_results["U"], abc_surrogate_results["D"]]
    ).T
...
def _(
    ...
    posterior_samples_mcmc = np.array(
        [mcmc_results_burned["U"], mcmc_results_burned["D"], mcmc_results_burned["sigma"]]
    ).T
...
def _(
    ...
    print(
        f"   U: mean={np.mean(abc_stochastic_results['U']):.3f}, std={np.std(abc_stochastic_results['U']):.3f}"
    )
    print(
        f"   D: mean={np.mean(abc_stochastic_results['D']):.3f}, std={np.std(abc_stochastic_results['D']):.3f}"
    )
    print(
        f"   Samples: {abc_stochastic_results['n_keep']:,} kept from {abc_stochastic_results['n_samples']:,}"
    )

    print(f"\n📊 ABC WITH SURROGATE MODEL:")
    print(
        f"   U: mean={np.mean(abc_surrogate_results['U']):.3f}, std={np.std(abc_surrogate_results['U']):.3f}"
    )
    print(
        f"   D: mean={np.mean(abc_surrogate_results['D']):.3f}, std={np.std(abc_surrogate_results['D']):.3f}"
    )
    print(
        f"   Samples: {abc_surrogate_results['n_keep']:,} kept from {abc_surrogate_results['n_samples']:,}"
    )

    print(f"\n📊 MCMC WITH SURROGATE MODEL:")
    print(
        f"   U: mean={np.mean(mcmc_results['U']):.3f}, std={np.std(mcmc_results['U']):.3f}"
    )
    print(
        f"   D: mean={np.mean(mcmc_results['D']):.3f}, std={np.std(mcmc_results['D']):.3f}"
    )
    print(
        f"   σ: mean={np.mean(mcmc_results['sigma']):.3f}, std={np.std(mcmc_results['sigma']):.3f}"
    )
...
def _(T, convert_D_to_P, np, simulator, time):
    ...
    def posterior_predictive_sample(
        parameter_samples, method_name, n_pred_samples=100, random_seed=42
    ):
        """
        ...
        for i, (U, D) in enumerate(zip(U_selected, D_selected)):
            if i % max(1, n_pred // 10) == 0 and i > 0:
                ...
                print(
                    f"   Progress: {i}/{n_pred} ({100 * i / n_pred:.1f}%) - ETA: {eta:.1f}s"
                )
        ...
        elapsed = time.time() - start_time
        print(
            f"✅ Predictive sampling for {method_name} completed in {elapsed:.1f} seconds"
        )
...
def _(np):
    ...
    def compute_prediction_intervals(
        predictions, method_name, percentiles=[2.5, 25, 50, 75, 97.5]
    ):
        """
        ...
        print(
            f"   Total agents - Mean: {global_stats['total_agents_mean']:.1f} ± {global_stats['total_agents_std']:.1f}"
        )
...
def _(Lx, np, plt):
    ...
    def plot_prediction_comparison(
        prediction_results_list, observed_data, method_names, colors
    ):
        """
        ...
        for i, (prediction_results, method_name, color) in enumerate(
            zip(prediction_results_list, method_names, colors)
        ):
            ...
            ax.fill_between(
                ...
                alpha=0.15,
                color=color,
                label=f"{method_name} 95% Interval",
            )
            ...
            ax.fill_between(
                ...
                alpha=0.25,
                color=color,
                label=f"{method_name} 50% Interval",
            )
...
def _(Lx, T, np, surrogate_pde_model):
    ...
    def posterior_predictive_pde_sample(
        parameter_samples, method_name, n_pred_samples=1000
    ):
        """
...
def _(Lx, np, plt):
    ...
    def plot_pde_prediction_envelope(
        envelope_results, observed_data, method_name, color="green"
    ):
        """
...
def _(
    ...
    abc_stochastic_pred_results = compute_prediction_intervals(
        predictions=abc_stochastic_predictions, method_name="ABC Stochastic"
    )
...
def _(
    ...
    abc_surrogate_pred_results = compute_prediction_intervals(
        predictions=abc_surrogate_predictions, method_name="ABC Surrogate"
    )
...
def _(
    ...
    mcmc_pred_results = compute_prediction_intervals(
        predictions=mcmc_predictions, method_name="MCMC"
    )
...
def _(
    ...
    for i, p in enumerate(prediction_results_list):
        fig_predictive = plot_prediction_comparison(
            [p],
        ...
        plt.savefig(
            f"notebooks/images/posterior_predictive_stochastic_{method_names[i]}.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.show()
...
def _(
    ...
    plt.savefig(
        f"notebooks/images/posterior_predictive_pde_ABC_stochastic.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.show()
...
def _(
    ...
    plt.savefig(
        f"notebooks/images/posterior_predictive_pde_ABC_surrogate.png",
        dpi=300,
        bbox_inches="tight",
    )
...
def _(
    ...
    mcmc_pde_predictions = posterior_predictive_pde_sample(
        parameter_samples=mcmc_results_burned, method_name="MCMC PDE", n_pred_samples=1000
    )
    ...
    plt.savefig(
        f"notebooks/images/posterior_predictive_pde_MCMC_surrogate.png",
        dpi=300,
        bbox_inches="tight",
    )
...
def _(
    ...
    for name, envelope_results in methods:
        ...
        print(
            f"   Total agents: {stats['total_agents_mean']:.1f} ± {stats['total_agents_std']:.1f}"
        )
