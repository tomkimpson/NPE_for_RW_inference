...
%DIF DEL old.tex        Fri Oct  3 07:58:11 2025
%DIF ADD template.tex   Fri Oct  3 08:15:22 2025

%DIF 3a3-10
\usepackage{hyperref}                 % Hyperlinks %DIF > 
\hypersetup{ %DIF > 
	hidelinks,                        % Hide link boxes %DIF > 
	colorlinks = true, %DIF > 
	citecolor = cyan, %DIF > 
	urlcolor = black, %DIF > 
	linkcolor = magenta %DIF > 
} %DIF > 
%DIF -------
\usepackage{arxiv}
\usepackage{amsmath}
...
%DIF 7d15
%DIF < \usepackage{hyperref}       % hyperlinks
%DIF -------
...
%DIF < % Hyperlinks (load near end to avoid conflicts)
%DIF < \usepackage{hyperref}                 % Hyperlinks
%DIF < \hypersetup{
%DIF < 	hidelinks,                        % Hide link boxes
%DIF < 	colorlinks = true,
%DIF < 	citecolor = cyan,
%DIF < 	urlcolor = black,
%DIF < 	linkcolor = magenta
%DIF < }
%DIF -------
...
%DIF 34c32
%DIF < \title{Amortized Inference for Spatiotemporal Stochastic Biological Models using Neural Posterior Estimation}
%DIF -------
\title{Rapid Parameter Inference for Spatiotemporal Stochastic Biological Models using Neural Posterior Estimation} %DIF > 
%DIF -------
...
\author[1]{%
%DIF 53c52
%DIF < 	\href{https://orcid.org/0000-0000-0000-0000}{\usebox{\orcid}\hspace{1mm}Kimpson\thanks{\texttt{kimpson email}}}%
%DIF -------
	\href{https://orcid.org/0000-0002-6542-6032}{\usebox{\orcid}\hspace{1mm}Tom Kimpson\thanks{Corresponding author: \texttt{tom.kimpson@unimelb.edu.au}}}% %DIF > 
%DIF -------
}
\author[1]{%
%DIF 56-57c55-56
%DIF < 	\href{https://orcid.org/0000-0000-0000-0000}{\usebox{\orcid}\hspace{1mm}Flegg}}
%DIF < 	
%DIF -------
	\href{https://orcid.org/0000-0002-8809-726X}{\usebox{\orcid}\hspace{1mm}Jennifer Flegg}% %DIF > 
} %DIF > 
%DIF -------
\author[2]{%
%DIF 59c58-59
%DIF < 	\href{https://orcid.org/0000-0000-0000-0000}{\usebox{\orcid}\hspace{1mm}Simpson}}
%DIF -------
	\href{https://orcid.org/0000-0001-6254-313X}{\usebox{\orcid}\hspace{1mm}Matthew J.\ Simpson}% %DIF > 
} %DIF > 
%DIF -------

%DIF 61-65c61-62
%DIF < \author[2]{%
%DIF < 	\href{https://orcid.org/0000-0000-0000-0000}{\usebox{\orcid}\hspace{1mm}Others...}}	
%DIF < 
%DIF < \affil[1]{Unimelb, etc}
%DIF < \affil[2]{QUT, etc}
%DIF -------
\affil[1]{School of Mathematics and Statistics, The University of Melbourne, Parkville, VIC 3010, Australia} %DIF > 
\affil[2]{School of Mathematical Sciences, Queensland University of Technology, Brisbane, QLD 4001, Australia} %DIF > 
%DIF -------
...
\begin{enumerate} 
	\item It is likelihood-free, circumventing intractability.
	 ...
	 \item It \DIFdelbegin \DIFdel{can ingest high-dimensional data directly (e.g., raw 2D images), preserving information and avoiding manual selection of summary statistics.
	 }%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{It }\DIFdelend provides amortized inference: after an initial, one-time training phase, it can generate posterior estimates for new observations almost instantaneously, overcoming the high per-inference cost of methods like ABC. 
	 \DIFaddbegin \item \DIFadd{It can ingest high-dimensional data directly (e.g., raw 2D images), preserving information and avoiding manual selection of summary statistics.
	 }\DIFaddend \end{enumerate}
The central thesis of this work is that NPE provides a robust, efficient, and direct framework for parameter inference in stochastic biological models. We demonstrate this by applying a state-of-the-art Sequential NPE (SNPE) workflow to a canonical agent-based random walk model of cell migration and proliferation. This type of model is a well-established tool for studying collective cell behaviour \citep[see e.g.][]{Simpson2025.05.25.656057} and serves as an ideal testbed for advanced inference methods. Our main contributions are to:
\DIFdelbegin \DIFdel{(i) introduce an amortized, }\DIFdelend \DIFaddbegin 

\begin{itemize}
	\item \textbf{\DIFadd{Modelling}} \DIFadd{We perform inference directly on the high-fidelity, agent-based random walk model. This }\DIFaddend simulator-native \DIFdelbegin \DIFdel{NPE pipeline for the migration and proliferation random-walk model; (ii)demonstrate that this NPE pipeline circumvents ABC tolerance tuning and }\DIFdelend \DIFaddbegin \DIFadd{approach entirely avoids the use of potentially unfaithful surrogate models (e.g., mean-field PDEs), thus eliminating }\DIFaddend surrogate-model \DIFdelbegin \DIFdel{bias while improving calibration and coverage; (iii) }\DIFdelend \DIFaddbegin \DIFadd{error and preserving the full stochastic realism of the biological system.
	}\item \textbf{\DIFadd{Inference}} \DIFadd{We deploy a modern Neural Posterior Estimation (NPE) framework as the core inference engine. This simulation-based, likelihood-free method directly learns the posterior distribution, overcoming the intractability of the likelihood function. Its amortized nature provides near-instantaneous posterior estimates for new observations after a single upfront training cost, bypassing the high per-inference expense and difficult tuning associated with traditional methods like ABC.
	}\item \textbf{\DIFadd{Learned Feature Extraction}} \DIFadd{We }\DIFaddend introduce a novel \DIFdelbegin \DIFdel{inference strategy that uses }\DIFdelend \DIFaddbegin \DIFadd{deep learning architecture by integrating }\DIFaddend a Convolutional Neural Network (CNN) \DIFdelbegin \DIFdel{to learn directly from the raw2D spatial datagenerated by the simulator.
	This approach avoids the information loss inherent in using 1D summary statistics (such as column counts), leading to more precise and reliable posterior estimates; (iv) Release an open}\DIFdelend \DIFaddbegin \DIFadd{as a learned feature extractor for the NPE pipeline. This enables the inference framework to learn informative representations automatically and directly from raw}\DIFaddend , \DIFaddbegin \DIFadd{two-dimensional spatial data, removing the need for manual, and potentially information-losing, data curation.
	}\item \textbf{\DIFadd{Integration}} \DIFadd{We demonstrate how these components form a unified, end-to-end pipeline that represents a new paradigm for inference in spatially-explicit stochastic models. By coupling the CNN feature extractor with the NPE inference engine, we perform Bayesian parameter estimation directly using the raw spatial output of the agent-based model. This synergy bypasses two major traditional bottlenecks: the need for hand-crafted summary statistics and the reliance on approximate surrogate models.
}\end{itemize}
\DIFadd{Additionally, we release an open-source, }\DIFaddend reproducible implementation \DIFdelbegin \DIFdel{with configuration files, plots, and posterior-predictive utilities to facilitate }\DIFdelend \DIFaddbegin \DIFadd{of our pipeline to facilitate its }\DIFaddend adoption and extension \DIFaddbegin \DIFadd{to other complex, spatially-structured biological models}\DIFaddend . While we use scratch assay experiments as our primary motivating application, the inference framework developed here is applicable to the broader class of spatially-explicit stochastic models commonly used throughout biology and ecology.
...
\DIFdelbegin \subsection{\DIFdel{Noise Model Specification}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Noise Model Specification}}\DIFaddend \label{sec:noise_model}
...
\DIFaddbegin \begin{figure}[h]
	\centering
	%DIF >  Placeholder for figure showing:
	%DIF >  (a) Initial agent distribution at t=0 on 2D lattice
	%DIF >  (b) Final agent distribution at t=100 on 2D lattice
	%DIF >  (c) Column counts N_i at t=0 (blue dots) with PDE solution Hu(x,t) (red curve)
	%DIF >  (d) Column counts N_i at t=100 (blue dots) with PDE solution Hu(x,t) (red curve)
	\includegraphics[width=\textwidth]{images/pde_comparison_figure2}
...
\DIFaddend To illustrate these theoretical considerations in practice, we reproduce a representative example from S\&P2025, which focuses on a stochastic agent-based model of a scratch barrier assay. In this model, agents representing biological cells are initially confined to a central region on a two-dimensional \DIFaddbegin \DIFadd{square }\DIFaddend lattice and subsequently spread outwards via an unbiased random walk. The \DIFdelbegin \DIFdel{resulting spatial distribution is quantified }\DIFdelend \DIFaddbegin \DIFadd{experimental setup, illustrated in Figure~\ref{fig:model_schematic}, mimics a barrier assay where cells are initially placed in a confined region and allowed to spread after barrier removal. The two-dimensional lattice has height $H=50$ and width $W=200$ (with $-100 \leq x \leq 100$), with lattice spacing $\Delta=1$ and discrete time step $\tau=1$. Zero-flux boundary conditions are applied at all edges. Initial conditions are set by occupying each site in the central region $|x| \leq 25$ with a single agent with probability $U=0.5$, as shown in Figure~\ref{fig:model_schematic}(a). At each time step, the system evolves through a random sequential update method where agents move to one of four nearest-neighbor sites with probability $P=0.7$. After 100 time steps, the population spreads symmetrically as shown in Figure~\ref{fig:model_schematic}(b).
}

\DIFadd{A key feature of this experimental design is the reduction of two-dimensional spatial data to one-dimensional measurements. Since the initial macroscopic density is uniform in the vertical ($y$) direction and boundary conditions preserve this symmetry, the resulting agent distribution remains independent of vertical location on average. This motivates quantifying the system }\DIFaddend by counting the number of agents \DIFdelbegin \DIFdel{in each vertical column, yielding noisy one-dimensional count data. Full details regarding the stochastic model, simulation parameters, and the generation of }\DIFdelend \DIFaddbegin \DIFadd{per column:
}\begin{equation}
	\DIFadd{N_i(t) = \sum_{j=1}^{J} n_{i,j}(t),
}\end{equation}
\DIFadd{where $n_{i,j}(t)$ denotes the integer number of agents at lattice site $(i,j)$ at time $t$, and $J$ is the number of lattice sites in the vertical direction. This dimensional reduction from 2D spatial data to 1D column counts is standard in cell biology scratch assays \mbox{%DIFAUXCMD
\citep{Jin2016}}\hskip0pt%DIFAUXCMD
, as it simplifies the characterization of spatiotemporal spreading while retaining the stochastic fluctuations inherent to the discrete process, as evident in Figure~\ref{fig:model_schematic}(c)-(d).
}

\DIFadd{For lattice-based random walk models of this type, }\DIFaddend the \DIFdelbegin \DIFdel{synthetic observation can be found in Appendix \ref{sec: synthetic_data_generation}. }\DIFdelend \DIFaddbegin \DIFadd{mean behaviour of agents in the continuum limit can be described by a PDE of the form \mbox{%DIFAUXCMD
\citep{Codling2008,Simpson2025.05.25.656057,Plank2025}}\hskip0pt%DIFAUXCMD
:
}\begin{equation}
	\DIFadd{\frac{\partial u}{\partial t} = -\frac{\partial J}{\partial x} + S,
}\end{equation}
\DIFadd{where $u(x,t)$ represents mean agent density, $J$ is the macroscopic flux, and $S$ denotes source terms from proliferation. For the simple case considered here (unbiased motion, no proliferation, no crowding), this reduces to the linear diffusion equation:
}\begin{equation}
	\DIFadd{\frac{\partial u}{\partial t} = D\frac{\partial^2 u}{\partial x^2},
}\end{equation}
\DIFadd{where $D = \lim_{\Delta,\tau \to 0} P\Delta^2/(4\tau)$ is the effective diffusion coefficient. Working with dimensionless units $\Delta=\tau=1$, this gives $D=P/4=0.175$ for our simulation with $P=0.7$.
}

\DIFadd{The continuous analogue of the column count $N_i$ under the PDE model is $Hu(x_i,t)$, where $x_i$ denotes the horizontal position of the $i$-th column and $H$ is the lattice height. Figure~\ref{fig:model_schematic}(c)-(d) compares the stochastic count data (blue dots) with this PDE solution (red curves), revealing that while the PDE captures mean trends, substantial stochastic fluctuations remain—a key motivation for carefully modeling observation noise.
}

\DIFaddend While S\&P2025 investigates a suite of model variations (e.g., incorporating proliferation\DIFdelbegin \DIFdel{and crowding}\DIFdelend \DIFaddbegin \DIFadd{, directional bias, and crowding effects}\DIFaddend ) and alternative PDE approximations, we deliberately focus on this simple \DIFaddbegin \DIFadd{unbiased, non-interacting }\DIFaddend case. It provides a clear baseline against which to compare our proposed NPE method, and we refer the reader to the original study for a fuller discussion of \DIFdelbegin \DIFdel{these }\DIFdelend more complex scenarios.
\DIFaddbegin 

\DIFaddend We aim to infer two key parameters that govern the model's behaviour: the initial occupation probability, $U$, and \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend effective diffusion coefficient, $D$. \DIFdelbegin \DIFdel{The diffusion coefficient, $D$, is a parameter of the continuum PDE model that can be used as a surrogate. The underlying agent-based model, however, is parameterised by a discrete transition probability, $P$. To facilitate a direct comparison between these different modelling frameworks, we reparameterise the results from the discrete model using the relationship $D=P/4$. 
We refer the reader to Appendix \ref{sec: synthetic_data_generation} for a full description of the PDE approximation.
}\DIFdelend \DIFaddbegin \DIFadd{For the synthetic data considered here, we use ground truth values $U=0.3$ and $D=0.175$. 
}\DIFaddend 
...
\begin{figure}[h!]
	...
	\begin{subfigure}{0.45\textwidth}
		\centering
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{images/corner_plot_abc_stochastic}
%DIFDELCMD < 		%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{images/corner_plot_abc_stochastic2}
		\DIFaddendFL \caption{Approximate posterior from ABC applied directly to the stochastic model. Computational constraints and information loss lead to a diffuse posterior for $D$}
		\label{fig:sub1}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.45\textwidth}
		\centering
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{images/corner_plot_abc_surrogate}
%DIFDELCMD < 		%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{images/corner_plot_abc_surrogate2}
		\DIFaddendFL \caption{Gaussian approximation of the posterior (Laplace approximation) via MLE on a deterministic surrogate model. This approach is computationally efficient but conditional on the surrogate's fidelity.}
		\label{fig:sub2}
	\end{subfigure}
	...
	\begin{subfigure}{0.55\textwidth}
		\centering
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{images/corner_plot_mcmc_surrogate}
%DIFDELCMD < 		%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{images/corner_plot_mcmc_surrogate2}
		\DIFaddendFL \caption{Posterior distribution from MCMC using the surrogate model with an assumed Gaussian noise model. This method jointly infers the model parameters alongside an additional noise variance parameter, $\sigma$.}
		\label{fig:sub3}
	\end{subfigure}
	\caption{Comparison of posterior distributions obtained using classical inference methods for a scratch assay model. Each approach demonstrates distinct trade-offs between computational efficiency, precision, and model assumptions. \DIFaddbeginFL \DIFaddFL{Orange lines indicate the true parameter values.}\DIFaddendFL }
...
At the core of NPE are normalizing flows, a class of flexible neural architectures that learn complex probability distributions through invertible transformations.\DIFdelbegin \DIFdel{A normalizing flow constructs a }\DIFdelend \DIFaddbegin \DIFadd{Normalizing flows have emerged as a powerful tool for density estimation in high-dimensional spaces \mbox{%DIFAUXCMD
\cite{rezende2015variational, papamakarios2021normalizing}}\hskip0pt%DIFAUXCMD
, offering significant advantages over traditional methods like mixture models or kernel density estimation when dealing with complex, multimodal distributions typical in biological inference problems. The fundamental idea behind normalizing flows is deceptively simple: construct a complex }\DIFaddend target distribution by transforming samples from a simple base distribution (typically a standard Gaussian) through a series of invertible, differentiable mappings. \DIFaddbegin \DIFadd{This approach can be understood as a sophisticated generalization of the familiar inverse transform sampling method, but extended to high dimensions through neural networks \mbox{%DIFAUXCMD
\cite{kobyzev2020normalizing}}\hskip0pt%DIFAUXCMD
. Where traditional change-of-variables techniques might struggle with complex distributions, normalizing flows leverage the representational power of deep learning while maintaining the mathematical rigor needed for proper density estimation.
}\DIFaddend 


Formally, let $u \in \mathbb{R}^D$ be a random variable with base density $p_u(u) = \mathcal{N}(0, I)$. The flow defines an invertible transformation $f_\phi: \mathbb{R}^D \rightarrow \mathbb{R}^D$ parameterized by neural network weights $\phi$. For conditional density estimation, this transformation is conditioned on the observation: $\theta = f_\phi(u; x)$. The invertibility requirement ensures that we can both generate samples (forward pass) and evaluate densities (inverse pass). The key insight is that complex distributions can be learned by composing simple transformations, each of which maintains invertibility and efficient Jacobian computation. This allows the network to learn highly flexible posterior shapes while maintaining computational tractability \DIFaddbegin \DIFadd{a crucial feature when inferring parameters in complex biological models where posteriors may exhibit non-Gaussian features such as multimodality, heavy tails, or curved degeneracies}\DIFaddend . The resulting approximate posterior density follows from the change of variables formula:
\begin{equation}
	q_\phi(\theta | x) = p_u(f_\phi^{-1}(\theta; x)) \left| \det J_{f_\phi^{-1}}(\theta; x) \right|
\end{equation}
where $J_{f_\phi^{-1}}$ denotes the Jacobian of the inverse transformation. Intuitively, the determinant of the Jacobian corrects for the local change in volume caused by the transformation, ensuring the resulting density is properly normalized. \DIFaddbegin \DIFadd{To understand this geometrically: when the transformation $f_{\phi}$
stretches space in some region, the determinant accounts for this stretching to ensure probability mass is conserved \mbox{%DIFAUXCMD
\cite{papamakarios2021normalizing}}\hskip0pt%DIFAUXCMD
. }\DIFaddend Modern flow architectures (e.g., coupling flows \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{dinh2016density}}\hskip0pt%DIFAUXCMD
}\DIFaddend , autoregressive flows \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{papamakarios2017masked}}\hskip0pt%DIFAUXCMD
}\DIFaddend ) are designed such that this determinant is computationally efficient to calculate\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{—often reducing to a sum or product of scalar terms rather than requiring explicit $D \times D$ matrix determinant computation. For readers familiar with traditional MCMC-based inference in biological models, normalizing flows offer a complementary approach: rather than exploring the posterior through iterative sampling, they learn an explicit parametric approximation that enables fast posterior evaluation and sampling once trained \mbox{%DIFAUXCMD
\cite{Cranmer2020FrontierSBI}}\hskip0pt%DIFAUXCMD
. This amortization—training once on simulated data, then instantly obtaining posteriors for new observations—makes them particularly attractive for scenarios requiring repeated inference, such as model selection, experimental design, or real-time data analysis.
}\DIFaddend 
...
\begin{enumerate}
	...
	\end{equation}
	\DIFaddbegin \DIFadd{This learns the transformation $f_\phi(u; x)$ that maps from a simple base distribution to the posterior distribution conditioned on the data.
	}\DIFaddend 
...
\DIFdelbegin \DIFdel{Training details and architectural specifications for the CNN are provided in Appendix \ref{ap:cnn}.
}\DIFdelend 
...
We assess the predictive performance of the 2D-trained model using the same posterior predictive checking procedure as in Section \ref{sec:posterior_checks}. As shown in \DIFdelbegin \DIFdel{Figure \ref{fig:npe_2d_predictive}, }\DIFdelend \DIFaddbegin \DIFadd{Figures \ref{fig:npe_2d_predictive}, \ref{fig:npe_2d_predictive_cnn}, }\DIFaddend the model generates predictions that are highly consistent with the observed data. The calibrated model accurately captures the spatial characteristics of the agent-based simulation, producing prediction intervals that are well-calibrated and physically consistent with the discrete nature of the data. \DIFdelbegin \DIFdel{Similar to the 1D case}\DIFdelend \DIFaddbegin \DIFadd{Importantly}\DIFaddend , the framework \DIFdelbegin \DIFdel{successfully captures the heteroscedastic nature of the uncertainty, demonstrating its robustness when }\DIFdelend \DIFaddbegin \DIFadd{captures the input-dependent (heteroscedastic) uncertainty—something that standard maximum-likelihood estimators under an i.i.d.\ noise assumption (as used in much of the mathematical biology literature) cannot represent, often leading to miscalibrated, homoscedastic intervals even when the underlying variability is spatially varying. As in the 1D case, the method remains robust when }\DIFaddend applied to high-dimensional \DIFaddbegin \DIFadd{(2D) }\DIFaddend spatial inputs.
...
\DIFdelbegin \DIFdel{\textcolor{magenta}{TK: perhaps experimentalists already do this. Again, need domain knowledge.}
}\DIFdelend 
...
\DIFdelbegin %DIFDELCMD < \newpage
%DIFDELCMD < %%%
\section{\DIFdel{Synthetic data generation}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{sec: synthetic_data_generation}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{\textcolor{magenta}{TK: TBD}
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The experimental configuration consists of a }\DIFdelend \DIFaddbegin \DIFadd{Our application to agent-based random walk models demonstrates these advantages in practice. On one-dimensional summary statistics, NPE matches or exceeds the precision of classical methods while requiring no manual tuning of tolerances or distance metrics. When extended to raw }\DIFaddend two-dimensional \DIFdelbegin \DIFdel{lattice with dimensions $H = 50$ and $W = 200$, where agents are initially placed within the region $|x| \leq 25$ with probability $U = 0.5$. The system evolves for 100 time steps with movement probability $P$ and proliferation probability $R$, corresponding to macroscopic diffusivity $D = P\Delta^2/(4\tau)$ and growth rate $r = R/\tau$ in the continuum limit. }%DIFDELCMD < 

%DIFDELCMD < %%%
\section{\DIFdel{Identifiability}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{ap:identifiability}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{\textcolor{magenta}{TK: TBD. Perhaps a short discussion on identifiability either here or in main text body. Might be out of scope.}
}%DIFDELCMD < 

%DIFDELCMD < %%%
\section{\DIFdel{NPE Implementation}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{ap:npe_implementaion}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{\textcolor{magenta}{TK: TBD. Could also point to config files in repo for reproducibility}
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Our specific implementation choices include:
}%DIFDELCMD < \begin{itemize}
%DIFDELCMD < 	\item %%%
\textbf{\DIFdel{Flow architecture:}} %DIFAUXCMD
\DIFdel{Neural spline flows with 5 coupling layers, 10 bins per spline, and embedding networks of 2 hidden layers with 50 units each }\DIFdelend \DIFaddbegin \DIFadd{spatial data through CNN-based feature extraction, the framework maintains comparable precision without requiring researchers to specify informative summary statistics a priori. This capability becomes critical for complex biological systems where crucial mechanistic information is encoded in spatial patterns that resist simple summarization}\DIFaddend .
\DIFdelbegin %DIFDELCMD < \item %%%
\textbf{\DIFdel{Training:}} %DIFAUXCMD
\DIFdel{Adam optimizer with learning rate $10^{-4}$, batch size 512, and early stopping based on validation loss. }%DIFDELCMD < \item %%%
\textbf{\DIFdel{Validation:}} %DIFAUXCMD
\DIFdel{20\% of simulations reserved for validation, with training stopped when validation loss increases for 10 consecutive epochs. }%DIFDELCMD < \end{itemize}
%DIFDELCMD < %%%
\DIFdelend 

\DIFdelbegin \section{\DIFdel{CNN implementation}} %DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{ap:cnn}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The broader implications extend beyond the specific models considered here. As biological research increasingly demands quantitative frameworks that can accommodate realistic stochasticity, spatial heterogeneity, and multi-scale dynamics, the ability to perform rigorous inference directly on high-fidelity simulators becomes essential. The framework we have developed provides a foundation for this next generation of biological modeling, where machine learning techniques can bridge the gap between model complexity and inferential tractability.
}\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdel{Our CNN architecture consists of :
}%DIFDELCMD < 

%DIFDELCMD < \begin{itemize}
%DIFDELCMD < 	\item %%%
\textbf{\DIFdel{Convolutional layers:}} %DIFAUXCMD
\DIFdel{Three convolutional layers with 32, 64, and 128 filters respectively, each with $3 \times 3$ kernels, followed by ReLU activations and $2 \times 2$ max pooling operations}\DIFdelend \DIFaddbegin \DIFadd{Nevertheless, important challenges remain. The substantial upfront computational cost of generating training data, the difficulty of ensuring robust performance outside the training distribution, and the limited theoretical guarantees compared to classical methods all require careful consideration. The architecture design choices and hyperparameter tuning introduce additional complexity that may present barriers to adoption. Future work should focus on reducing these barriers through improved architectures, more efficient training algorithms, and the development of practical guidelines for applying these methods across diverse biological systems}\DIFaddend .

\DIFdelbegin %DIFDELCMD < \item %%%
\textbf{\DIFdel{Dimensionality reduction:}} %DIFAUXCMD
\DIFdel{The CNN compresses the $H \times W = 10,000$ dimensional input to a 256-dimensional embedding that captures relevant spatial features while remaining computationally tractable.
	}\DIFdelend \DIFaddbegin \DIFadd{We provide an open-source implementation of our complete pipeline}\footnote{\href{https://github.com/tomkimpson/NPE_for_RW_inference}{\DIFadd{github.com/tomkimpson/NPE\_for\_RW\_inference}}} \DIFadd{to facilitate adoption and enable the community to extend these methods to increasingly complex spatially-structured models. As computational resources continue to expand and neural network architectures become more sophisticated, we anticipate that simulation-based inference frameworks like NPE will become the standard approach for parameter estimation in stochastic biological systems. 
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \item %%%
\textbf{\DIFdel{Fully connected layers:}} %DIFAUXCMD
\DIFdel{The flattened convolutional features are passed through two fully connected layers (512 and 256 units) with ReLU activations and dropout (rate = 0.3) for regularization. }\DIFdelend \DIFaddbegin \newpage
%DIF > 
%DIF > \appendix
%DIF > 
%DIF > \section{NPE Implementation}\label{ap:npe_implementaion}
%DIF > 
%DIF > We implement Sequential Neural Posterior Estimation (SNPE) using neural spline flows (NSF) as the density estimator, following the framework established by \cite{2020JOSS....5.2505T}. For the baseline 1D inference problem, where observations consist of column-wise marginalizations of the spatial distribution, we employ the following architecture and training configuration:
%DIF > \begin{itemize}
%DIF > 	\item \textbf{Data representation:} Column counts (200 values per observation, corresponding to lattice width $L_x = 200$).
%DIF > 	\item \textbf{Flow architecture:} Neural spline flows (NSF) with 8 coupling layers and 128 hidden features.
%DIF > 	\item \textbf{Training:} Adam optimizer with learning rate $10^{-4}$, batch size 128, training for up to 300 epochs with early stopping based on validation loss.
%DIF > 	\item \textbf{Validation:} 10\% of simulations reserved for validation, with training stopped when validation loss increases for 10 consecutive epochs.
%DIF > 	\item \textbf{SNPE rounds:} 10 rounds with 2,000 simulations per round (10,000 total initial simulations for first round).
%DIF > 	\item \textbf{Convergence:} Sequential rounds terminated when the convergence metric (based on posterior mean and standard deviation changes) falls below 0.001.
%DIF > 	\item \textbf{Implementation:} Built using the SBI library \citep{2020JOSS....5.2505T} with PyTorch backend, trained on NVIDIA A100-SXM4-80GB GPU (85.1 GB memory).
%DIF > \end{itemize}
%DIF > 
%DIF > 
%DIF > 
%DIF > \section{CNN implementation} \label{ap:cnn}
%DIF > 
%DIF > To process the full 2D spatial information, we developed a convolutional neural network (CNN) that serves as an embedding network within the NSF architecture. This embedding network compresses the high-dimensional spatial observations into a tractable latent representation that preserves the relevant spatial structure while enabling efficient density estimation. The design incorporates modern architectural elements including residual connections, batch normalization, and progressive downsampling to ensure stable training and effective feature extraction.
%DIF > 
%DIF > The complete CNN architecture comprises the following components:
%DIF > 
%DIF > \begin{itemize}
%DIF > 	\item \textbf{Convolutional layers:} Four progressive downsampling convolutional stages with 32, 64, 128, and 256 filters respectively. Each stage uses $3 \times 3$ kernels with stride 2, batch normalization, ReLU activations, and residual connections implemented via $1 \times 1$ convolutional shortcuts.
%DIF > 	
%DIF > 	\item \textbf{Dimensionality reduction:} The CNN compresses the $50 \times 200 = 10,000$ dimensional input through progressive downsampling followed by global average pooling, producing a 256-dimensional embedding that captures relevant spatial features while remaining computationally tractable.
%DIF > 	
%DIF > 	\item \textbf{Fully connected layers:} The pooled convolutional features are passed through three fully connected layers (512, 256, and 256 units) with ReLU activations and dropout (rate = 0.05) for regularization.
%DIF > 	
%DIF > 	\item \textbf{Normalization:} Per-sample normalization is applied to the input data before processing, with mean subtraction and standard deviation scaling to stabilize training.
%DIF > 	
%DIF > 	\item \textbf{Translation invariance:} The convolutional architecture with residual connections naturally handles the translation-invariant nature of the spreading process, learning features that are consistent across spatial locations.
%DIF > \end{itemize}
%DIF > 
%DIF > 
%DIF > The integration of the CNN embedding network into the SNPE framework necessitates several important modifications to both the network architecture and training procedure relative to the 1D baseline. These adjustments reflect the substantially increased dimensionality of the input space and the additional complexity introduced by the CNN's learnable parameters. The key differences in our 2D implementation are:
%DIF > 
%DIF > \begin{itemize}
%DIF > 	\item \textbf{Data representation:} Full 2D spatial grid (10,000 values per observation, corresponding to $L_y \times L_x = 50 \times 200$), representing the complete spatial distribution of agents rather than compressed column counts.
%DIF > 	
%DIF > 	\item \textbf{Flow architecture:} Neural spline flows (NSF) with 5 coupling layers and 256 hidden features (compared to 8 coupling layers and 128 hidden features for 1D data).
%DIF > 	
%DIF > 	\item \textbf{Increased simulation budget:} Due to the higher-dimensional input space, we use 6,000 simulations per SNPE round (compared to 2,000 for 1D) to ensure adequate coverage of the joint parameter-data space.
%DIF > 	
%DIF > 	\item \textbf{Extended training time:} The CNN embedding network requires additional optimization iterations. We use a learning rate of $5 \times 10^{-5}$ (compared to $10^{-4}$ for 1D) with batch size 512, training for up to 150 epochs with early stopping after 40 consecutive epochs of no improvement (compared to 10 for 1D).
%DIF > 	
%DIF > 	\item \textbf{SNPE rounds:} 8 rounds with 6,000 simulations per round (5,000 total initial simulations for first round).
%DIF > 	
%DIF > 	\item \textbf{Convergence:} Sequential rounds terminated when the convergence metric falls below 0.01 (relaxed from 0.001 for 1D due to the increased complexity of the 2D inference problem).
%DIF > 	
%DIF > 	\item \textbf{Memory considerations:} Each 2D observation requires significantly more memory ($50 \times 200 = 10,000$ values vs. 200 for column counts), necessitating careful batch size selection and data loading strategies.
%DIF > 	
%DIF > 	\item \textbf{Implementation:} Same computational infrastructure as 1D (SBI library, PyTorch, NVIDIA A100-SXM4-80GB GPU), with the CNN embedding network integrated directly into the NSF density estimator.
%DIF > \end{itemize}
%DIF > 
%DIF > 
%DIF > 
%DIF > All results are fully reproducible using the configuration files and code available in the project repository at \texttt{https://github.com/tomkimpson/NPE\_for\_RW\_inference}.
\DIFaddend 

\DIFdelbegin %DIFDELCMD < \item %%%
\textbf{\DIFdel{Translation invariance:}} %DIFAUXCMD
\DIFdel{The convolutional architecture naturally handles the translation-invariant nature of the spreading process, learning features that are consistent across spatial locations.
}%DIFDELCMD < \end{itemize}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Training NPE with 2D datarequires several adjustments compared to the 1D case:
}%DIFDELCMD < 
...
%DIFDELCMD < 	\item %%%
\item%DIFAUXCMD
\textbf{\DIFdel{Increased simulation budget:}} %DIFAUXCMD
\DIFdel{Due to the higher-dimensional input space, we use 15,000 simulations per SNPE round (compared to 10,000 for 1D) to ensure adequate coverage of the joint parameter-data space. }%DIFDELCMD < 

%DIFDELCMD < 	\item %%%
\item%DIFAUXCMD
\textbf{\DIFdel{Extended training time:}} %DIFAUXCMD
\DIFdel{The CNN embedding network requires additional optimization iterations. We use a learning rate of $5 \times 10^{-5}$ with batch size 256, training for up to 500 epochs with early stopping.
	}%DIFDELCMD < 

%DIFDELCMD < 	\item %%%
\item%DIFAUXCMD
\textbf{\DIFdel{Memory considerations:}} %DIFAUXCMD
\DIFdel{Each 2D observation requires significantly more memory ($50 \times 200 = 10,000$ values vs. 201 for column counts), necessitating careful batch size selection and data loading strategies. 
}
