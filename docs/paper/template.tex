...
\subsection{NPE Implementation}

We implement NPE using neural spline flows with the following specifications:

\begin{itemize}
	\item \textbf{Prior distributions:} We adopt uniform priors consistent with S\&P2025: $D \sim \mathcal{U}(0.05, 0.40)$, $U \sim \mathcal{U}(0.30, 0.70)$, and when applicable, $r \sim \mathcal{U}(0, 0.02)$.
	
	\item \textbf{Data representation:} Unlike S\&P2025's use of 1D column counts, we explore two data modalities: (i) the standard column count vector $N_i$ for direct comparison, and (ii) the full 2D spatial configuration of agents.
	
	\item \textbf{Network architecture:} We employ a neural spline flow with XXX coupling layers, XXX bins per spline, and embedding networks consisting of XXX hidden layers with XXX units each.
