...
\begin{table}[htbp]
	...
	\begin{tabular}{@{}lcc@{}}
		...
		\bottomrule
	\end{tabular}
	\caption{Summary of key hyperparameters for the single-round NPE pipeline in the 1D and 2D configurations. All models were trained on an NVIDIA A100-SXM4-80GB GPU using the \texttt{sbi} library \citep{Boelts2025sbi} with a PyTorch backend. {\color{red}\textbf{Note for TK:} need to rerun 1D results with 50k to prevent confounds. Current results use 10k.}}
...
Posterior predictive checks confirm that the inferred model reproduces the observed data, with 94.8\% of observations falling within the 95\% prediction intervals. Unlike surrogate-based approaches, the prediction intervals are physically consistent---respecting non-negativity and naturally capturing heteroscedastic uncertainty (Appendix~\ref{sec:appendix_ppc}).
...
The CNN-based approach (Figure~\ref{fig:npe_original_2d}) recovers a posterior with medians $D = 0.156^{+0.026}_{-0.023}$
 and $U = 0.294^{+0.011}_{-0.010}$. As summarised in Table~\ref{tab:data_comparison}, leveraging the full spatial data yields a posterior with a precision that is highly comparable to that obtained using curated 1D summary statistics, cf.\ Section~\ref{sec:1d_results}.
...
Posterior predictive checks for the 2D model similarly confirm well-calibrated predictions that capture the spatial characteristics of the agent-based simulation (Appendix~\ref{sec:appendix_ppc}).
...
\begin{table}[htbp]
	...
	\begin{tabular}{llcc}
		...
		Surrogate + MLE/Laplace & 1D Column counts & $0.314^{+0.004}_{-0.004}$ & $0.201^{+0.024}_{-0.024}$ \\
		...
		\bottomrule
	\end{tabular}
	\caption{Comparison of inference results across methods and data representations. All values show posterior medians with 68\% credible intervals ($x^{+a}_{-b}$); classical method posteriors are extracted from the corner plots in Figure~\ref{fig:three_figures}. The MCMC method additionally infers a noise variance nuisance parameter $\sigma$ (not shown). The surrogate-based methods (Laplace, MCMC) yield biased estimates of $D$, reflecting systematic error from the mean-field approximation.}
...
NPE with 1D column counts recovers $U = 0.600^{+0.349}_{-0.464}$, $P = 0.801^{+0.163}_{-0.283}$, and $R = 0.048^{+0.017}_{-0.010}$ (Figure~\ref{fig:model_B_1d}). All three true values fall within the 95\% credible intervals. The posterior for $U$ is broad and left-skewed, reflecting a partial degeneracy between initial density and proliferation: higher initial occupancy can mimic the effect of increased growth when only a single time point is observed. Despite this degeneracy, both $P$ and $R$ remain well-constrained. With 2D spatial data, NPE recovers $U = 0.345^{+0.290}_{-0.223}$, $P = 0.777^{+0.158}_{-0.129}$, and $R = 0.053^{+0.013}_{-0.009}$ (Figure~\ref{fig:model_B_2d}). The 2D representation narrows the $U$ posterior considerably compared to 1D, suggesting that spatial pattern information helps break the $U$--$R$ degeneracy.

Biologically, the ability to jointly infer $U$, $P$, and $R$ is valuable: in wound healing assays the seeding density may not be precisely controlled, and NPE can recover it alongside the motility and proliferation parameters from a single experimental snapshot.
...
\begin{figure}[htbp]
	...
	\end{subfigure}
	\caption{Posterior distributions for Model B (proliferation) with true values $U = 0.5$, $P = 0.7$, $R = 0.05$. The $U$--$R$ degeneracy is visible in the 1D posterior, while $P$ and $R$ remain well-constrained.}
...
NPE with 1D column counts recovers $U = 0.597^{+0.205}_{-0.281}$, $P = 0.657^{+0.113}_{-0.112}$, $\rho = 0.515^{+0.223}_{-0.116}$, and $R = 0.052^{+0.008}_{-0.005}$ (Figure~\ref{fig:model_C_1d}). Despite the four-parameter space, $R$ is recovered with $\sim$13\% relative uncertainty, and the $P$--$\rho$ anti-correlation mirrors the degeneracy observed in Model~A. With 2D spatial data, NPE recovers $U = 0.354^{+0.125}_{-0.119}$, $P = 0.743^{+0.066}_{-0.059}$, $\rho = 0.390^{+0.039}_{-0.037}$, and $R = 0.055^{+0.006}_{-0.004}$ (Figure~\ref{fig:model_C_2d}). The 2D posteriors are substantially tighter, particularly for $P$ and $\rho$, demonstrating the additional constraining power of spatial information in this four-parameter setting.

Biologically, Model C captures the interplay of initial cell density, cell motility, directional migration (e.g.\ chemotaxis), and cell division---the four key processes governing wound healing dynamics. The ability to infer all four parameters simultaneously from a single experimental observation, without relying on surrogate approximations or noise model specifications, demonstrates NPE's value for complex biological systems where classical surrogate-based methods become increasingly difficult to justify.
...
\begin{figure}[htbp]
	...
	\end{subfigure}
	\caption{Posterior distributions for Model C (bias + proliferation) with true values $U = 0.5$, $P = 0.7$, $\rho = 0.5$, $R = 0.05$. Despite the four-parameter space and the lack of a well-validated surrogate likelihood, NPE recovers all parameters with well-calibrated uncertainty estimates.}
...
Table~\ref{tab:diagnostics} summarises the diagnostic results for both 1D and 2D posteriors across all models. The C2ST scores are consistently close to $0.5$ across all models, parameters, and data representations, indicating that the learned posteriors are nearly indistinguishable from the true posteriors and that the density estimators have been well-trained. The TARP area-under-the-curve (ATC) metric passes for Model~A 2D (ATC $= 0.08$) and shows moderate deviations for the remaining models.

For the 1D posteriors, the KS test passes for all parameters in the Original and Model~B posteriors, indicating good marginal calibration. In Model~A, $U$ fails the KS test, $P$ passes marginally ($p = 0.07$), and $\rho$ passes comfortably. In Model~C, $P$ and $R$ fail the KS test, with the SBC rank ECDF (Figure~\ref{fig:sbc_plots}d) bowing below the diagonal for $R$, indicating an excess of low ranks. This signature is characteristic of mild overconfidence: the learned posteriors for $R$ are sharp and precise (as seen in the corner plots, Figures~\ref{fig:model_B_posteriors}--\ref{fig:model_C_posteriors}), but slightly too narrow, so that the true value falls in the upper tail of the posterior more often than expected. This is distinct from the broad, uninformative posteriors that would signal a lack of identifiability. The overconfidence likely reflects the nonlinear and relatively flat mapping from $R$ to the observable column counts at small proliferation rates: modest changes in $R$ produce only subtle differences in the data, making it difficult for the normalizing flow to accurately learn the posterior width.

The 2D posteriors exhibit broadly similar calibration patterns (dashed lines in Figure~\ref{fig:sbc_plots}), with C2ST scores remaining near 0.5 throughout. The KS test is somewhat more sensitive in the 2D setting: the Original model shows failures for both $U$ and $P$; Model~A fails for all three parameters ($U$, $P$, and $\rho$); Model~B shows a marginal failure for $R$; and Model~C fails for $U$ and $\rho$ while $P$ and $R$ pass. These failures likely reflect the challenge of learning accurate posterior widths in the higher-dimensional observation space. This tendency to underestimate posterior variance is not unique to NPE but is a recognised challenge across all simulation-based inference methods \citep{hermans2022crisis}. Notably, \cite{hermans2022crisis} find that amortized approaches such as NPE tend to be more conservative (less overconfident) than their non-amortized counterparts, and are more simulation-efficient when accounting for hyper-parameter tuning and diagnostic evaluation. Importantly, we note that the KS test is highly sensitive to subtle deviations in the tails of the rank distribution, which may not affect practical inference quality. The C2ST diagnostic---which directly tests whether posterior samples are distinguishable from the true posterior---passes consistently across all models and parameters, providing stronger evidence that the learned posteriors are well-calibrated for practical use. Increasing the training budget or employing sequential refinement strategies could potentially improve KS calibration further, but we note that the miscalibration observed here is mild, with curves remaining close to the DKW confidence bands.
...
\begin{table}[htbp]
	...
	\begin{tabular}{@{}llcccccc@{}}
		...
		\textbf{Model} & \textbf{Param.} & \textbf{KS $p$} & \textbf{C2ST} & \textbf{ATC} & \textbf{KS $p$} & \textbf{C2ST} & \textbf{ATC} \\
		\midrule
		Original & $U$ & 0.08 & 0.59 & \multirow{2}{*}{0.48} & 0.0001\rlap{$^*$} & 0.58 & \multirow{2}{*}{$-0.41$} \\
		         & $P$ & 0.08 & 0.58 & & 0.02\rlap{$^*$} & 0.59 & \\
		\midrule
		A        & $U$    & 0.0001\rlap{$^*$} & 0.57 & \multirow{3}{*}{$-0.24$} & 0.0000\rlap{$^*$} & 0.64 & \multirow{3}{*}{0.08} \\
		         & $P$    & 0.07   & 0.56 & & 0.0000\rlap{$^*$} & 0.56 & \\
		         & $\rho$ & 0.96   & 0.57 & & 0.02\rlap{$^*$} & 0.58 & \\
		\midrule
		B        & $U$ & 0.60 & 0.58 & \multirow{3}{*}{$-0.31$} & 0.11 & 0.57 & \multirow{3}{*}{$-0.36$} \\
		         & $P$ & 0.45 & 0.59 & & 0.41 & 0.57 & \\
		         & $R$ & 0.32 & 0.57 & & 0.009\rlap{$^*$} & 0.59 & \\
		\midrule
		C        & $U$    & 0.13 & 0.56 & \multirow{4}{*}{$-0.73$} & 0.004\rlap{$^*$} & 0.58 & \multirow{4}{*}{$-0.12$} \\
		         & $P$    & 0.002\rlap{$^*$} & 0.56 & & 0.07 & 0.58 & \\
		         & $\rho$ & 0.25 & 0.59 & & 0.0000\rlap{$^*$} & 0.58 & \\
		         & $R$    & 0.0000\rlap{$^*$} & 0.56 & & 0.22 & 0.58 & \\
		\bottomrule
	\end{tabular}
	\caption{Diagnostic summary for 1D and 2D NPE posteriors across all models. KS $p$-values marked with $^*$ indicate failure ($p < 0.05$). C2ST scores near 0.5 indicate well-learned posteriors. TARP ATC values near zero indicate good calibration.}
	\label{tab:diagnostics}
\end{table}

Figure~\ref{fig:sbc_plots} shows SBC rank plots for all four models, with 1D posteriors (solid lines) and 2D posteriors (dashed lines) overlaid on the same axes. Uniform rank distributions indicate well-calibrated posteriors, while deviations reveal systematic over- or under-confidence in specific parameter directions.
...
\begin{figure}[htbp]
	...
	\begin{subfigure}{0.48\textwidth}
		...
		\caption{Model C (bias + proliferation)}
		\label{fig:sbc_model_C}
	\end{subfigure}
	\caption{Simulation-based calibration (SBC) rank ECDF plots for all four models. Solid lines show 1D NPE posteriors; dashed lines show 2D NPE (CNN) posteriors. A uniform rank distribution (diagonal within the shaded 99\% DKW confidence bands) indicates well-calibrated posteriors; an ECDF bowing below the diagonal indicates mild overconfidence. (a)~Original model: 1D posteriors are well-calibrated for both parameters; 2D posteriors show slight deviations for $U$ and $P$. (b)~Model A: $U$ deviates in both representations; $P$ passes marginally and $\rho$ is well-calibrated in 1D, while all three parameters fail in 2D. (c)~Model B: all parameters are well-calibrated in 1D; $R$ shows marginal deviation in 2D. (d)~Model C: the four-parameter space shows mild miscalibration for $P$ and $R$ in 1D, and for $U$ and $\rho$ in 2D.}
	\label{fig:sbc_plots}
\end{figure}
...
\begin{table}[htbp]
	...
	\begin{tabular}{@{}llccc@{}}
		...
		\midrule
		\multirow{2}{*}{Original} & $U$ & 0.300 & $0.298^{+0.009}_{-0.008}$ & $0.294^{+0.011}_{-0.010}$ \\
		                          & $D$ & 0.175 & $0.164^{+0.024}_{-0.024}$ & $0.156^{+0.026}_{-0.023}$ \\
		\midrule
		\multirow{3}{*}{A (Bias)} & $U$    & 0.500 & $0.498 \pm 0.014$ & $0.508 \pm 0.030$ \\
		                          & $P$    & 0.700 & $0.627 \pm 0.156$ & $0.801^{+0.170}_{-0.200}$ \\
		                          & $\rho$ & 0.500 & $0.593 \pm 0.170$ & $0.416^{+0.147}_{-0.073}$ \\
		\midrule
		\multirow{3}{*}{B (Proliferation)} & $U$ & 0.500 & $0.600^{+0.349}_{-0.464}$ & $0.345^{+0.290}_{-0.223}$ \\
		                                   & $P$ & 0.700 & $0.801^{+0.163}_{-0.283}$ & $0.777^{+0.158}_{-0.129}$ \\
		                                   & $R$ & 0.050 & $0.048^{+0.017}_{-0.010}$ & $0.053^{+0.013}_{-0.009}$ \\
		\midrule
		\multirow{4}{*}{C (Bias + Prolif.)} & $U$    & 0.500 & $0.597^{+0.205}_{-0.281}$ & $0.354^{+0.125}_{-0.119}$ \\
		                                    & $P$    & 0.700 & $0.657^{+0.113}_{-0.112}$ & $0.743^{+0.066}_{-0.059}$ \\
		                                    & $\rho$ & 0.500 & $0.515^{+0.223}_{-0.116}$ & $0.390^{+0.039}_{-0.037}$ \\
		                                    & $R$    & 0.050 & $0.052^{+0.008}_{-0.005}$ & $0.055^{+0.006}_{-0.004}$ \\
		\bottomrule
	\end{tabular}
	\caption{Comprehensive NPE inference results across all models. Values show posterior medians with 68\% credible intervals. For the original model, $D = P/4$ is reported for comparison with classical methods. All models use broad uniform priors on the unit interval for probability parameters and $R \sim \mathcal{U}(0.001, 0.2)$ for the proliferation rate.}
...
\begin{table}[htbp]
	...
	\begin{tabular}{@{}llcccc@{}}
		...
		\midrule
		\multirow{3}{*}{B (Prolif.)} & $U$ & 0.500 & $0.5458 \pm 0.0213$ & $0.7903 \pm 0.0274$ & 5/5 \\
		                              & $P$ & 0.700 & $0.7702 \pm 0.0159$ & $0.3990 \pm 0.0368$ & 5/5 \\
		                              & $R$ & 0.050 & $0.0493 \pm 0.0005$ & $0.0261 \pm 0.0020$ & 5/5 \\
		\midrule
		\multirow{4}{*}{C (Bias + Prolif.)} & $U$    & 0.500 & $0.5418 \pm 0.0170$ & $0.7550 \pm 0.0151$ & 5/5 \\
		                                    & $P$    & 0.700 & $0.6477 \pm 0.0151$ & $0.4499 \pm 0.0316$ & 5/5 \\
		                                    & $\rho$ & 0.500 & $0.5193 \pm 0.0238$ & $0.5029 \pm 0.0306$ & 5/5 \\
		                                    & $R$    & 0.050 & $0.0539 \pm 0.0004$ & $0.0244 \pm 0.0010$ & 5/5 \\
		\bottomrule
	\end{tabular}
	\caption{Reproducibility across random seeds. For each model and parameter, we report the mean $\pm$ standard deviation of the posterior mean and the 95\% credible interval width across 5 independent training runs. Coverage indicates the fraction of seeds for which the true value falls within the 95\% credible interval. All 1D NPE with 50{,}000 training simulations. {\color{red}\textbf{Note for TK:} seed study results for Original and Model A are missing---only B and C exist in \texttt{results/seed\_study/}. Need to run seed study for Original and A, and rerun B and C with 50k sims (current results use 10k).}}
...
\begin{table}[htbp]
	...
	\begin{tabular}{@{}llccc@{}}
		...
		\bottomrule
	\end{tabular}
		\caption{Computational runtimes for various inference methods. NPE costs are separated into simulation generation (50{,}000 training simulations on 8~CPU workers) and neural network training (on an A100 GPU). Simulation time is independent of output format (1D column counts vs.\ 2D spatial grids), as the underlying stochastic simulation is identical. Simulation generation dominates the cost, accounting for $\sim$74\% of the total for the original model and over 96\% for the exclusion models~A--C. Once trained, NPE inference for 5{,}000 posterior samples takes under one second. {\color{red}\textbf{Note for TK:} runtime values in table body still reflect 10k sims; update after rerun with 50k.}}	\label{tab:runtimes}
\end{table}
...
The value of spatial inference is demonstrated concretely by the extended models. Model A (directional bias) creates anisotropic spreading patterns where agents preferentially migrate in one direction, producing spatial asymmetries that are collapsed when column counts sum across rows. The 2D CNN has access to these directional features, and while the $P$--$\rho$ degeneracy persists in both representations (Section~\ref{sec:model_a}), the spatial approach provides a representation that naturally accommodates anisotropy without requiring the analyst to design bias-aware summary statistics. The concrete benefits of spatial data emerge most clearly for models with proliferation. In Model~B, the 2D posterior for $U$ is substantially narrower than its 1D counterpart, suggesting that spatial patterns of daughter cells help break the $U$--$R$ degeneracy that plagues column-count-based inference (Section~\ref{sec:model_b}). In Model~C, the improvements are even more pronounced: 2D posteriors for $P$ and $\rho$ are roughly two to four times tighter than 1D posteriors (Table~\ref{tab:all_results}), demonstrating that spatial information provides genuine additional constraining power in the most complex model. More broadly, the cost-benefit calculation shifts decisively toward spatial approaches for models where crucial information is encoded in patterns that resist simple summarization: agent-agent interactions, chemotaxis, cell-cell adhesion, heterogeneous environments, and multi-species dynamics all generate spatial structure that 1D projections discard.
...
\subsection{1D Column Count Predictions}
...
\subsection{2D Spatial Predictions}
