...
\begin{enumerate} 
	...
	\item \textbf{Noise-model misspecification:} The use of a deterministic surrogate creates the problem of specifying a noise model to connect its output to noisy data. A misspecified choice can lead to non-physical predictions and poorly calibrated uncertainties. 
...
\begin{table}[htbp]
	...
	\begin{tabular}{lcc}
		\toprule
		\textbf{Data Type} & \textbf{Posterior for $U$} &  \textbf{Posterior for $D$}  \\
		\midrule
1D Column counts  & $0.298^{+0.009}_{-0.008}$ & $0.164^{+0.024}_{-0.024}$\\
2D Spatial data   & $0.290^{+0.008}_{-0.009}$ & $0.169^{+0.026}_{-0.023}$\\
		\bottomrule
	\end{tabular}
	\caption{Comparison of NPE performance using different data representations. Values show posterior medians with 68\% credible intervals ($x^{+a}_{-b}$). Inference using the 2D spatial data yields comparable posterior precision to the 1D summary statistics.}
