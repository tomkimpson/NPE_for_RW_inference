...
All NPE analyses in this work use the \texttt{sbi} Python package \cite{Boelts2025sbi}, which provides a well-documented implementation of state-of-the-art simulation-based inference algorithms. We validate all trained posteriors using the diagnostic protocol recommended by \cite{Cranmer2020FrontierSBI}: simulation-based calibration (SBC) to check rank uniformity, and TARP coverage tests to assess calibration across credibility levels (Section~\ref{sec:diagnostics}). We employ neural spline flows \cite{durkan2019neural} as our density estimator architecture. Neural spline flows construct invertible transformations from monotonic rational-quadratic spline segments, providing highly expressive density estimation with analytically tractable Jacobians and stable training dynamics. Additional implementation details can be found in the project GitHub repository \footnote{\href{github.com/tomkimpson/NPE_for_RW_inference}{https://github.com/tomkimpson/NPE\_for\_RW\_inference}}.
...
\paragraph{Parameter Inference Results.}
...
\paragraph{Predictive Performance.}\label{sec:posterior_checks}
...
\paragraph{Parameter Inference Results.}
Figure~\ref{fig:npe_2d_posterior} shows the joint posterior distribution inferred from the 2D spatial data, overlaid with the 1D column count posterior from Section~\ref{sec:1d_results} for direct comparison. The CNN-based approach recovers a posterior with means $D = 0.169^{+0.026}_{-0.023}$
