...
Model C combines directional bias and proliferation, representing the most biologically realistic scenario in our suite. As in Model~B, the initial occupation probability is fixed at $U = 0.5$, leaving three free parameters: the motility probability $P$, the directional bias $\rho$, and the proliferation rate $R$, with true values $P = 0.7$, $\rho = 0.5$, $R = 0.05$. Crucially, no tractable likelihood is available for this model---the interplay of bias, proliferation, and exclusion effects makes classical surrogate-based inference infeasible. This model therefore represents the strongest case for simulation-based inference being \emph{necessary}, not merely convenient.

NPE with 1D column counts recovers $P = 0.656^{+0.256}_{-0.259}$, $\rho = 0.507^{+0.395}_{-0.158}$, and $R = 0.052^{+0.007}_{-0.005}$ (Figure~\ref{fig:model_C_1d}). Despite the three-parameter space, $R$ is recovered with $\sim$12\% relative uncertainty, and the $P$--$\rho$ anti-correlation mirrors the degeneracy observed in Model~A. With 2D spatial data, NPE recovers $P = 0.802^{+0.155}_{-0.201}$, $\rho = 0.381^{+0.254}_{-0.082}$, and $R = 0.051^{+0.005}_{-0.005}$ (Figure~\ref{fig:model_C_2d}). The proliferation rate remains well-constrained despite the increased dimensionality.
...
\begin{figure}[htbp]
	...
	\end{subfigure}
	\caption{Posterior distributions for Model C (bias + proliferation) with true values $P = 0.7$, $\rho = 0.5$, $R = 0.05$. Despite the three-parameter space and absence of a tractable likelihood, NPE recovers all parameters with well-calibrated uncertainty estimates.}
...
\begin{table}[htbp]
	...
	\begin{tabular}{@{}llcccccc@{}}
		...
		\midrule
		C        & $P$    & 0.86 & 0.55 & \multirow{3}{*}{$-0.72$} & 0.0000\rlap{$^*$} & 0.62 & \multirow{3}{*}{$-0.79$} \\
		         & $\rho$ & 0.32 & 0.56 & & 0.0000\rlap{$^*$} & 0.64 & \\
		         & $R$    & 0.00\rlap{$^*$} & 0.59 & & 0.02\rlap{$^*$} & 0.58 & \\
		\bottomrule
	\end{tabular}
...
\begin{table}[htbp]
	...
	\begin{tabular}{@{}llccc@{}}
		...
		\midrule
		\multirow{3}{*}{C (Bias + Prolif.)} & $P$    & 0.700 & $0.656^{+0.256}_{-0.259}$ & $0.802^{+0.155}_{-0.201}$ \\
		                                    & $\rho$ & 0.500 & $0.507^{+0.395}_{-0.158}$ & $0.381^{+0.254}_{-0.082}$ \\
		                                    & $R$    & 0.050 & $0.052^{+0.007}_{-0.005}$ & $0.051^{+0.005}_{-0.005}$ \\
		\bottomrule
	\end{tabular}
