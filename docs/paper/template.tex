...
Biologically, Model C captures the interplay of initial cell density, cell motility, directional migration (e.g.\ chemotaxis), and cell division---the four key processes governing wound healing dynamics. The ability to infer all four parameters simultaneously from a single experimental observation, without a tractable likelihood function, demonstrates NPE's unique value for complex biological systems where traditional inference methods are unavailable.
...
\begin{figure}[htbp]
	...
	\end{subfigure}
	\caption{Posterior distributions for Model C (bias + proliferation) with true values $U = 0.5$, $P = 0.7$, $\rho = 0.5$, $R = 0.05$. Despite the four-parameter space and absence of a tractable likelihood, NPE recovers all parameters with well-calibrated uncertainty estimates.}
...
\begin{table}[htbp]
	...
	\begin{tabular}{@{}llcccccc@{}}
		...
		\midrule
		Original & $U$ & 0.08 & 0.58 & \multirow{2}{*}{0.32} & 0.0001\rlap{$^*$} & 0.59 & \multirow{2}{*}{0.18} \\
		         & $P$ & 0.03\rlap{$^*$} & 0.58 & & 0.36 & 0.58 & \\
		\midrule
		A        & $U$    & 0.0001\rlap{$^*$} & 0.57 & \multirow{3}{*}{$-0.78$} & 0.0000\rlap{$^*$} & 0.62 & \multirow{3}{*}{0.27} \\
		         & $P$    & 0.05\rlap{$^*$}   & 0.56 & & 0.81 & 0.58 & \\
		         & $\rho$ & 0.03\rlap{$^*$}   & 0.57 & & 0.0000\rlap{$^*$} & 0.58 & \\
		\midrule
		B        & $U$ & 0.71 & 0.57 & \multirow{3}{*}{$-0.23$} & 0.17 & 0.60 & \multirow{3}{*}{$-0.38$} \\
		         & $P$ & 0.01\rlap{$^*$} & 0.58 & & 0.41 & 0.58 & \\
		         & $R$ & 0.50 & 0.59 & & 0.0004\rlap{$^*$} & 0.58 & \\
		\midrule
		C        & $U$    & 0.55 & 0.59 & \multirow{4}{*}{$-0.59$} & 0.11 & 0.57 & \multirow{4}{*}{$-0.64$} \\
		         & $P$    & 0.0007\rlap{$^*$} & 0.57 & & 0.0000\rlap{$^*$} & 0.57 & \\
		         & $\rho$ & 0.22 & 0.59 & & 0.0000\rlap{$^*$} & 0.56 & \\
		         & $R$    & 0.0000\rlap{$^*$} & 0.59 & & 0.11 & 0.58 & \\
		\bottomrule
	\end{tabular}
...
\begin{figure}[htbp]
	...
	\end{subfigure}
	\caption{Simulation-based calibration (SBC) rank ECDF plots for all four models. Solid lines show 1D NPE posteriors; dashed lines show 2D NPE (CNN) posteriors. A uniform rank distribution (diagonal within the shaded 99\% DKW confidence bands) indicates well-calibrated posteriors; an ECDF bowing below the diagonal indicates mild overconfidence. (a)~Original model: both representations show good calibration, with the 2D posterior for $U$ slightly deviating. (b)~Model A: 1D posteriors deviate for all parameters due to the $P$--$\rho$ degeneracy; 2D posteriors show similar patterns for $U$ and $\rho$ but improved calibration for $P$. (c)~Model B: $U$ and $R$ are well-calibrated in both representations; $P$ marginally deviates in 1D while $R$ deviates in 2D. (d)~Model C: the four-parameter space shows mild miscalibration for $P$ and $R$ in 1D, and for $P$ and $\rho$ in 2D, while $U$ is well-calibrated in both representations.}
	\label{fig:sbc_plots}
\end{figure}
...
\begin{table}[htbp]
	...
	\begin{tabular}{@{}llccc@{}}
		...
		\midrule
		\multirow{3}{*}{A (Bias)} & $U$    & 0.500 & $0.498 \pm 0.014$ & $0.489 \pm 0.012$ \\
		                          & $P$    & 0.700 & $0.627 \pm 0.156$ & $0.801^{+0.086}_{-0.083}$ \\
		                          & $\rho$ & 0.500 & $0.593 \pm 0.170$ & $0.409^{+0.047}_{-0.039}$ \\
		\midrule
...
\begin{table}[htbp]
	...
	\begin{tabular}{@{}llccc@{}}
		...
		\midrule
		ABC & Stochastic Simulator & -- & -- & $\sim$1200 \\
		ABC & Surrogate Model & -- & -- & $\sim$2 \\
		MCMC & Surrogate Model & -- & -- & $\sim$1 \\
		\midrule
		NPE & Original & $\sim$515 & $\sim$175 & $<$0.1 \\
		NPE & Model~A & $\sim$4325 & $\sim$135 & $<$0.1 \\
		NPE & Model~B & $\sim$4925 & $\sim$125 & $<$0.1 \\
		NPE & Model~C & $\sim$4855 & $\sim$170 & $<$0.1 \\
		\bottomrule
	\end{tabular}
		\caption{Computational runtimes for various inference methods. NPE costs are separated into simulation generation (10{,}000 training simulations on 8~CPU workers) and neural network training (on an A100 GPU). Simulation time is independent of output format (1D column counts vs.\ 2D spatial grids), as the underlying stochastic simulation is identical. Simulation generation dominates the cost, accounting for $\sim$74\% of the total for the original model and $\sim$97\% for the exclusion models~A--C. Once trained, NPE inference is near-instantaneous.}	\label{tab:runtimes}
\end{table}
