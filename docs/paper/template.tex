...
\title{Amortized Inference for Stochastic Biological Models using Neural Posterior Estimation}
...
\begin{abstract}
Parameter inference for stochastic models is essential for quantitative biology but is hampered by intractable likelihoods. Agent-based random walk models, used widely in ecology and cell biology, exemplify this challenge, often forcing researchers to use surrogate models or likelihood-free methods like Approximate Bayesian Computation (ABC) that introduce biases and tuning difficulties. Here, we overcome these limitations using Neural Posterior Estimation (NPE), a simulation-based, machine learning inference framework that learns the full posterior distribution. Once trained, NPE provides amortized, near-instantaneous posterior estimates. We demonstrate this framework in two settings: first, using one-dimensional summary statistics (column counts), and second, via a novel convolutional neural network (CNN) that learns directly from raw two-dimensional spatial data. On synthetic barrier assay data, our NPE pipeline matches or exceeds the precision of established baselines at a substantially reduced computational cost per inference. The spatial CNN approach further reduces posterior uncertainty by 8-9\%, highlighting a critical trade-off between statistical precision and the increased computational cost. Our analysis provides a clear framework for deciding when the full spatial data is necessary versus when summary statistics suffice, a finding with direct implications for experimental design. We provide an open-source implementation of our pipeline to facilitate its adoption and extension to more complex, spatially-structured biological models.
\end{abstract}
...
The invertibility requirement ensures that we can both generate samples (forward pass) and evaluate densities (inverse pass). The key insight is that complex distributions can be learned by composing simple transformations, each of which maintains invertibility and efficient Jacobian computation. This allows the network to learn highly flexible posterior shapes while maintaining computational tractability.

The resulting approximate posterior density follows from the change of variables formula:
\begin{equation}
...
The training data requirements scale with problem complexity. Typically, $N = 10^4 - 10^6$ simulations suffice for moderate-dimensional parameter spaces ($d \leq 10$), though higher-dimensional problems may require larger datasets. Standard techniques for preventing overfitting include validation set monitoring, early stopping, and dropout regularization in the embedding networks.

Crucially, this procedure is \textit{amortized}; the computationally expensive training phase is performed only once, after which posterior inference for any new observation is nearly instantaneous. This represents a significant advantage over methods like ABC, which require fresh simulations for each new dataset.
...
\subsection{Application to Barrier Assay Data: 1D Column Count Analysis}

We demonstrate NPE's effectiveness by applying it to the same synthetic barrier assay data used in Section \ref{sec:example_classical_results}. To ensure direct comparison with \emph{S\&P2025}'s classical methods, we first implement NPE using identical 1D column count summary statistics $N_i$, representing the number of agents in each vertical column of the lattice.

\subsubsection{Experimental Setup}

Following the protocol of \SP, we generate synthetic observations from a stochastic random walk model with true parameters $P = 0.7$ and $U = 0.3$, corresponding to diffusivity $D = P/4 = 0.175$ in the continuum limit. The resulting data consists of a 201-dimensional vector of column counts. 

We perform inference directly on the stochastic model parameters $P$ and $U$, then transform to $D$ via the relationship $D = P/4$ for comparison with PDE-based approaches. We implement SNPE with the following specifications:

\begin{itemize}
	\item \textbf{Prior distributions:} Broad uniform priors covering the full parameter space: $P \sim \mathcal{U}(0.0, 1.0)$ and $U \sim \mathcal{U}(0.01, 1.0)$.
	
	\item \textbf{Training protocol:} SNPE with 3 rounds, using 10,000 simulations per round. The first round samples from the prior, while subsequent rounds use the posterior from the previous round as the proposal distribution.
	
	\item \textbf{Network architecture:} Neural spline flow with 5 coupling layers, 10 bins per spline, and embedding networks of 2 hidden layers with 50 units each.
\end{itemize}

\subsubsection{Parameter Inference Results}
Figure \ref{fig:npe_1d_posterior} presents the posterior distribution obtained via NPE. NPE recovers a posterior distribution with means $P = 0.698 \pm 0.037$ and $U = 0.301 \pm 0.022$ (mean $\pm$ standard deviation), corresponding to $D = 0.175 \pm 0.009$ after transformation. The results demonstrate excellent agreement with the true parameter values ($P = 0.7$, $U = 0.3$) and substantially tighter credible intervals than ABC.
...
\begin{figure}[h!]
	...
	\begin{subfigure}{0.45\textwidth}
		\centering
		\includegraphics[width=\textwidth]{images/NPE_corner_plot_1d_D}
		\caption{}
		\label{fig:sub11}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.45\textwidth}
		\centering
		\includegraphics[width=\textwidth]{images/example_predictive_plot_for_paper_1D}
		\caption{}
		\label{fig:sub22}
	\end{subfigure}
\end{figure}
...
%	\caption{Posterior distribution from NPE using 1D column count data. The contours show 68\% and 95\% credible regions, with the true parameter values (red cross) falling well within the high-probability region. Training required 30,000 total simulations across 3 SNPE rounds.}
...
\begin{enumerate}
	\item \textbf{Improved precision:} NPE posteriors show 60\% tighter credible intervals compared to ABC ($10^5$ simulations), without requiring tolerance tuning or summary statistic selection.
	
	\item \textbf{Elimination of surrogate bias:} Unlike surrogate-based methods, NPE learns directly from the stochastic simulator, avoiding systematic bias from mean-field approximations. The posterior mode aligns closely with true parameter values.
	
	\item \textbf{Computational efficiency:} After initial training requiring 2.5 CPU hours, NPE provides posterior samples in under 10 seconds for new observations, compared to 12+ hours for ABC with $10^5$ simulations.
\end{enumerate}
...
The NPE-based predictions demonstrate several advantages over classical approaches:
...
\begin{itemize}
	...
	\item \textbf{Proper calibration:} 94.8\% of true observations fall within the 95\% prediction intervals, indicating well-calibrated uncertainty quantification.
	
	\item \textbf{Heteroscedastic uncertainty:} Prediction intervals naturally adapt to local agent density, appropriately widening in regions of high stochastic variability.
\end{itemize}
...
\begin{figure}[h]
	...
	\label{fig:npe_1d_posterior_predictive}
\end{figure}
...
While column counts provide a natural summary of the spreading process, they necessarily discard information about the vertical distribution of agents. In principle, the spatial correlation structure within the 2D configuration contains additional information about the underlying parameters. For instance, clustering patterns, local density fluctuations, and spatial correlations may help distinguish between different combinations of $P$ and $U$ that produce similar column counts.

Traditional inference methods require hand-crafted summary statistics, making it challenging to leverage such high-dimensional spatial information. NPE, however, can learn directly from raw data through automatic feature extraction, potentially improving inference precision. While the simple random walk models considered here are likely well-captured by 1D summary statistics, this approach demonstrates the broader potential of NPE for more complex biological systems where spatial structure is critical.

For example, models incorporating agent-agent interactions, complex boundary conditions, spatially heterogeneous environments, or multi-species dynamics would generate spatial patterns that cannot be adequately summarized by simple column counts. In such cases, the ability to learn directly from full spatial data becomes essential for accurate parameter inference. Our framework provides a foundation that can be readily extended to these more sophisticated modeling scenarios without requiring manual specification of appropriate summary statistics.

\subsection{Architectural Adaptations for 2D Data}

Processing 2D spatial data requires architectural modifications to handle the increased dimensionality and spatial structure. We treat the lattice configuration as a binary image of size $H \times W = 50 \times 200$, where each pixel indicates agent presence/absence. To extract relevant features while maintaining computational tractability, we employ a convolutional neural network \citep{lecun1998,krizhevsky2012,goodfellow2016} (CNN) as the embedding network within our normalizing flow architecture.
...
Figure \ref{fig:npe_2d_posterior} shows the posterior distribution obtained using full 2D spatial data, compared with the 1D column count results from Section \ref{sec:NPE}.

%\begin{figure}[h]
%	\centering
%	\includegraphics[width=0.8\textwidth]{images/NPE_corner_plot_2d_D_v2}
%	\caption{Posterior distribution from NPE using full 2D spatial data. The contours show 68\% and 95\% credible regions. Training required 45,000 total simulations across 3 SNPE rounds with CNN embedding networks.}
...
\begin{figure}[h!]
	...
	\begin{subfigure}{0.45\textwidth}
		\centering
		\includegraphics[width=\textwidth]{images/NPE_corner_plot_2d_D_v2}
		\caption{}
		\label{fig:sub11}
	\end{subfigure}
	\hfill
	\begin{subfigure}{0.45\textwidth}
		\centering
		\includegraphics[width=\textwidth]{images/example_predictive_plot_for_paper_2D}
		\caption{}
		\label{fig:sub22}
	\end{subfigure}
\end{figure}
...
Table \ref{tab:data_comparison} provides a quantitative comparison of NPE performance using different data representations:

\begin{table}[h]
	\centering
	\begin{tabular}{lcccc}
		\hline
		Data Type & Posterior SD ($P$) & Posterior SD ($U$) & Training Time & Simulations \\
		\hline
		1D Column counts & 0.037 & 0.022 & 2.5 hours & 30,000 \\
		2D Spatial data & 0.034 & 0.020 & 6.8 hours & 45,000 \\
		\hline
	\end{tabular}
	\caption{Comparison of NPE performance using different data representations. The 2D spatial data provides modest improvements in posterior precision (8\% reduction in posterior uncertainty for $P$, 9\% for $U$) at the cost of increased computational requirements.}
...
\subsection{Understanding Learned Spatial Features}

Our analysis of the CNN-based spatial approach reveals insights into how neural networks automatically extract relevant features from complex biological data. Through activation visualization, we observe that intermediate CNN layers develop sensitivity to edge characteristics, local density gradients, and spatial clustering patterns that distinguish different parameter regimes. These learned representations focus primarily on spreading front characteristics and overall spatial extent, which aligns well with the theoretical understanding of how movement probability and initial density affect pattern formation in random walk models.

The automatically extracted features encompass local density variations that help distinguish genuine stochastic fluctuations from systematic patterns indicative of specific parameter values. Edge characteristics provide information about spreading rates that correlate with movement probability, while overall spatial extent and fill fraction relate to initial density parameters. Importantly, the network also learns to recognize spatial correlation lengths that reflect the underlying diffusivity properties of the system.

While these learned features demonstrate the CNN's ability to extract problem-relevant information without manual specification, our results also highlight an important limitation: for the simple random walk models considered here, the spatial features largely capture information already present in one-dimensional column counts. This finding underscores the importance of matching methodological complexity to problem requirements and suggests that more sophisticated spatial analysis should be reserved for systems where spatial structure genuinely contains additional information.

\subsection{When Spatial Approaches Become Essential}

\textcolor{magenta}{TK: need to query Jen/Mat for domain knowledge bio inputs here. I think for these simple RW systems the CNN is probably overkill, but there are likely systems where spatial info becomes more useful.}

The modest improvements observed for simple random walks should not obscure the broader potential of spatial inference approaches. Our framework becomes increasingly valuable as biological complexity increases beyond the idealized scenarios considered here. Models incorporating agent-agent interactions through exclusion effects, chemotaxis, or cell-cell adhesion generate intricate spatial patterns that cannot be adequately captured by one-dimensional projections. Similarly, heterogeneous environments with spatially varying growth rates, physical barriers, or chemical gradients create complex spatial signatures that contain crucial parameter information.

Multi-species dynamics involving competition, predation, or cooperation produce emergent spatial organization that reflects the underlying interaction parameters. Collective behaviors such as swarming, flocking, or tissue morphogenesis involve coordinated movements that are fundamentally spatial phenomena requiring spatial analysis for proper characterization. In these more complex scenarios, traditional summary statistics would discard essential information, making CNN-based approaches not merely advantageous but necessary for accurate parameter inference.

The transition from summary statistics to full spatial analysis represents more than a technical upgrade; it enables the investigation of biological systems previously beyond the reach of quantitative analysis. Complex multicellular systems, ecological communities, and developmental processes all exhibit spatial organization that encodes information about underlying mechanisms. Our computational framework provides the foundation for tackling these sophisticated modeling challenges as they arise in cutting-edge biological research.

\subsection{Implications for Experimental Design and Practice}
\textcolor{magenta}{TK: as above, require some domain-specific knowledge here}

The demonstrated capability to learn from full spatial data carries significant implications for how biological experiments are designed and analyzed. Traditional protocols often reduce rich spatial information to simplified scalar measurements such as wound closure rates or total cell counts, primarily for computational convenience rather than biological necessity. Our results suggest that preserving full spatial information during data collection could substantially improve parameter identifiability, particularly for complex systems where spatial structure encodes mechanistic information.

Time-lapse microscopy naturally generates high-resolution spatial data that could be directly analyzed using our framework without requiring manual feature extraction or subjective choice of summary statistics. This approach could unlock new insights from existing datasets and inform the design of future experiments that capitalize on spatial information content. However, the computational requirements and modest improvements observed for simple models indicate that spatial analysis should be strategically deployed when biological systems exhibit clear spatial organization, when traditional summary statistics prove inadequate, or when computational resources permit the additional complexity.

The trade-off between statistical precision and computational cost requires careful consideration in practical applications. For the simple random walk model, full spatial data reduced posterior uncertainty by approximately 8-9% for both parameters while requiring nearly three-fold increase in training time and 50% more simulations. This modest gain suggests that for simpler systems, well-chosen summary statistics remain highly effective and computationally efficient. However, for more complex biological systems where crucial information is encoded in spatial patterns that resist simple summarization, this cost-benefit calculation shifts decisively toward spatial approaches.

\subsection{Computational Scalability and Future Directions}

The CNN-based spatial analysis introduces several computational considerations that will influence the method's applicability to large-scale problems. Memory requirements scale quadratically with spatial resolution, potentially limiting application to very high-resolution imaging data without careful optimization. Architecture selection for CNN embedding networks requires problem-specific tuning of depth, width, and kernel sizes, adding complexity to the analysis pipeline. The combined optimization of feature extraction and density estimation can be more challenging than simpler approaches, requiring careful hyperparameter selection and potentially longer training times.
