...
Figure~\ref{fig:sbc_plots} shows SBC rank plots for all four models. Uniform rank distributions indicate well-calibrated posteriors, while deviations reveal systematic over- or under-confidence in specific parameter directions.
...
\begin{figure}[htbp]
	...
	\end{subfigure}
	\caption{Simulation-based calibration (SBC) rank plots for all four models. Uniform distributions (within the shaded confidence bands) indicate well-calibrated posteriors. (a)~Original model shows good calibration for both $U$ and $P$. (b)~Model A shows deviations for $U$, $P$, and $\rho$, reflecting the $P$--$\rho$ degeneracy identified in Section~\ref{sec:model_a}. (c)~Model B shows good calibration for $P$ with mild deviations for $R$, consistent with the KS test results in Table~\ref{tab:diagnostics}. (d)~Model C shows similar patterns to Model A for $P$ and $\rho$, with additional deviations for $R$ consistent with the marginal KS failure reported in Table~\ref{tab:diagnostics}.}
	\label{fig:sbc_plots}
\end{figure}
