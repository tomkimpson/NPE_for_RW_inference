...
class RandomWalkSimulatorNumpy:
    """
    2D lattice random walk simulator (Optimized with NumPy). 
    """
    
    def __init__(self, Lx: int, Ly: int, initial_region_half_width: Optional[int] = None):
        """Initializes the simulator with lattice dimensions."""
        self.Lx = Lx
        self.Ly = Ly
        self.initial_region_half_width = initial_region_half_width or Lx // 4
        ...
        # Pre-calculate boundaries for frequent use
        self.x_min = -(self.Lx // 2)
        self.x_max = (self.Lx // 2) - (1 - self.Lx % 2)
        self.y_min = 0
        self.y_max = self.Ly - 1
    
    def initialize_lattice(self, U: float, random_seed: Optional[int] = None) -> np.ndarray:
        """
        Vectorized initialization of agent positions using NumPy.
        
        Returns:
        --------
        np.ndarray
            (N, 2) array of initial agent positions (x, y).
        """
        if not 0 < U <= 1:
            raise ValueError("Occupancy probability U must be in (0, 1]")
        ...
        # Define the initial grid of possible positions
        x_range = np.arange(-self.initial_region_half_width, self.initial_region_half_width + 1)
        y_range = np.arange(self.Ly)
        grid_x, grid_y = np.meshgrid(x_range, y_range)
        all_sites = np.vstack([grid_x.ravel(), grid_y.ravel()]).T

        # Which sites are occupied?
        num_sites = len(all_sites)
        occupied_mask = np.random.random(size=num_sites) < U
        
        return all_sites[occupied_mask]

    def simulate_step(self, positions: np.ndarray, P: float) -> np.ndarray:
        """
        Execute one time step using an optimized NumPy-based approach.
        """
        if not 0 <= P <= 1:
            raise ValueError("Movement probability P must be in [0, 1]")
        ...
        # Potential move deltas: right, left, up, down
        deltas = np.array([[1, 0], [-1, 0], [0, 1], [0, -1]])

        # Random sequential update loop (structure is preserved)
        for _ in range(Q):
            # 1. Select a random agent
            idx = np.random.randint(0, Q)
            
            # 2. Decide if it moves
            if np.random.random() < P:
                x, y = new_positions[idx]
                
                # 3. Efficiently determine valid moves using a boolean mask
                # This avoids creating a new Python list in every iteration
                valid_mask = np.array([
                    x < self.x_max,    # Can move right?
                    x > self.x_min,    # Can move left?
                    y < self.y_max,    # Can move up?
                    y > self.y_min     # Can move down?
                ])
                
                valid_deltas = deltas[valid_mask]
                
                # 4. If any moves are possible, choose one and update position
                if valid_deltas.shape[0] > 0:
                    move_idx = np.random.randint(valid_deltas.shape[0])
                    new_positions[idx] += valid_deltas[move_idx]
        
        return new_positions
    
    def simulate(self, U: float, P: float, T: int, random_seed: Optional[int] = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        Run a complete simulation. Now returns NumPy arrays.
        """
        if T < 0:
            raise ValueError("Number of time steps T must be non-negative")
        ...
        # Note: The return types are now consistently np.ndarray
        return column_counts, initial_positions, positions
    
    def get_column_counts(self, positions: np.ndarray) -> np.ndarray:
        """
        Vectorized counting of agents in each column using np.bincount.
        """
        if positions.shape[0] == 0:
            return np.zeros(self.Lx, dtype=int)
        
        # 1. Extract all x-coordinates in one operation
        x_coords = positions[:, 0]
        
        # 2. Shift coordinates to be non-negative for array indexing
        # This maps x from [-Lx/2, Lx/2] to [0, Lx-1]
        array_indices = x_coords - self.x_min
        
        # 3. Use bincount for highly efficient counting
        counts = np.bincount(array_indices, minlength=self.Lx)
...
# Plotting functions. These should have their own module really
